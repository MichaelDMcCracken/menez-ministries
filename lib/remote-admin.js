...
async function publishChangesRemotely(publisher, payload = {}) {
  const nextData = validateSermonDataset(payload.data);
  const { libraryHtml } = await loadRemoteRepositoryState(publisher);
  const commitMessage = normalizeString(payload.commitMessage) || defaultCommitMessage('Publish staged sermon changes');
  return publishDataAndSite(publisher, nextData, libraryHtml, commitMessage);
}
